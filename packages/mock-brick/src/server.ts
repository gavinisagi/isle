...
export interface BrickServer {
  ...
  // Chaos: forcibly drop all SSE connections (tests host reconnect). / 混沌:强断所有 SSE 连接(测 host 重连)
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && url.startsWith(SSE_PATH)) {
      ...
      if (Number.isFinite(lastId)) {
        ...
        // Fresh connect: deliver current state so state-ful bricks render immediately. / 新连接:立即给当前状态,让状态型 brick 即刻渲染
        res.write(latest);
      }
