...
// Snap grid on drag end (Q19). / 拖动结束的吸附网格(Q19)
const GRID = 8;
...
export function ExpandedPanel({ bricks }: { bricks: BrickView[] }): JSX.Element {
  ...
  const startDrag = (id: string, index: number, e: ReactMouseEvent): void => {
    ...
    const onMove = (m: globalThis.MouseEvent): void => {
      setPositions((prev) => ({
        ...prev,
        [id]: { x: Math.max(0, start.x + (m.clientX - startX)), y: Math.max(0, start.y + (m.clientY - startY)) },
      }));
    };
    const onUp = (): void => {
      document.removeEventListener('mousemove', onMove);
      document.removeEventListener('mouseup', onUp);
      // Snap to grid + persist once, on drag end. / 拖动结束吸附到网格并持久化一次
      setPositions((prev) => {
        const p = prev[id];
        if (!p) return prev;
        const snapped = { x: snap(p.x), y: snap(p.y) };
        window.isle?.setCardPosition(id, snapped.x, snapped.y);
